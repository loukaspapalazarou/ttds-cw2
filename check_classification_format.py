    ...
    with open(path) as infile:
        ...
        for i, line in enumerate(infile.readlines()):
            ...
            if len(parts) != 14:
                if verbose:
                    print(
                        f"Incorrect number of columns in line {i+1}. Expected:14, Got:{len(parts)}"
                    )
                correct = False
            else:
                (
                ...
                if (system, split) in seen_system_split_combos:
                    if verbose:
                        print(
                            f"duplicate (system,split) pair on line {i+1}:{(system,split)}"
                        )
                    correct = False
                ...
                for j, score in enumerate(
                    [pq, rq, fq, po, ro, fo, pn, rn, fn, pm, rm, fm]
                ):
                    try:
